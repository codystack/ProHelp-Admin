...
export default function UpdateCategoryForm({ setOpen, selected }) {
...
//   const profile = useSelector((state) => state.profile.profileData);
  ...
  const [previewImage, setPreviewImage] = React.useState(selected?.row?.image);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      skills: selected?.row?.skills,
      description: selected?.row?.description,
    },
    validationSchema,
    onSubmit: async (values) => {
      dispatch(setLoading(true));
      if (mfile) { // IMage changed
        try {
                ...
                getDownloadURL(uploadTask.snapshot.ref)
                  .then(async (downloadURL) => {
                    ...
                    const res = await APIService.update(
                      "/profession/update", selected?.row?.id,
                      payload
                    );
